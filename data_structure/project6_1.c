...
int front = -1;                 // 뒷 자리를 지킴. 전역 변수로 설정해준 front. 생성된지 제일 오래된 자료의 위치를 가리킨다. dequeue시 front는 +1 됨. 처음엔 -1로 초기화
int rear = -1;                  // 선두. 전역 변수로 설정해준 rear. 가장 최근에 enqueue된 위치이다. enqueue시 rear +1됨. 처음엔 -1로 초기화
