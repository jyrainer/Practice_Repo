...
def find_pay(P, Q, R, S ,W):
    """
    Args:
        P: A사, 1L당 요금
        Q: B사, RL 이하의 기본 요금
        R: B사, 기본요금만 부과되는 상한선, 이상 시 추가요금
        S: B사, R리터 이상 초과시 부과되는 1L당 요금
        W: 한달간 사용하는 수도 토탈
    Return:
        pay (int): A나 B중에 더 저렴하게 사용할 요금
    """
    return min(
        P*W,
        Q if W <= R else (Q+ (W-R)*S)
    )
...
for test_case in range(1, T + 1):
    ...
    print(f"#{test_case} {find_pay(P,Q,R,S,W)}")